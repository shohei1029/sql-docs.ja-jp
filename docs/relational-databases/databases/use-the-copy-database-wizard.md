---
description: データベース コピー ウィザードの使用
...
11. **[転送元のデータベース ファイルの場所]** ページ: **転送元のサーバーでのファイル共有** の省略記号ボタンをクリックして、指定したフォルダーの場所に移動します。  たとえば、フォルダーの場所 `D:\MSSQL13.MSSQLSERVER\MSSQL\DATA` は、 **転送元のサーバーでのファイル共有** の `\\Server1\D$\MSSQL13.MSSQLSERVER\MSSQL\DATA` を使用します。  **[次へ]** をクリックします。
